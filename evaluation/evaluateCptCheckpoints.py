#!/usr/bin/env python3
"""Evaluate all saved full-phase CPT checkpoints for a run, including intermediate Trainer checkpoints that need tokenizer metadata borrowed from the run final directory."""
...
DEFAULT_KRIKRI_REPORT_CACHE = REPO_ROOT / "artifacts/reports/greek_mmlu_krikri_eval.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Krikri reference model id or local path passed through to evaluate_greek_mmlu.py.",
    )
    parser.add_argument(
        "--evaluate-krikri",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Opt in to evaluating the Krikri reference model alongside the base and trained checkpoints.",
    )
    parser.add_argument(
        "--krikri-report-cache",
        default=str(DEFAULT_KRIKRI_REPORT_CACHE),
        help="Persistent Krikri-model cache report reused across checkpoint evaluations.",
    )
    parser.add_argument(
        "--use-krikri-report-cache",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Reuse the persistent Krikri-model cache across checkpoint evaluations.",
    )
    parser.add_argument(
        "--refresh-krikri-report-cache",
        action="store_true",
        help="Force a fresh Krikri evaluation instead of reusing the cached report.",
    )
    parser.add_argument(
        "--evaluation-script",
...
def build_eval_command(
    ...
    if args.evaluate_krikri:
        command.extend(
            [
                "--krikri-model",
                args.krikri_model,
                "--krikri-report-cache",
                str(Path(args.krikri_report_cache).expanduser().resolve()),
                "--use-krikri-report-cache" if args.use_krikri_report_cache else "--no-use-krikri-report-cache",
            ]
        )
    else:
        command.append("--no-evaluate-krikri")
    if args.attn_implementation:
        command.extend(["--attn-implementation", args.attn_implementation])
    if args.trust_remote_code:
        command.append("--trust-remote-code")
    if args.evaluate_krikri and args.refresh_krikri_report_cache:
        command.append("--refresh-krikri-report-cache")
...
def summary_record(candidate: CheckpointCandidate, report: dict[str, Any], report_path: Path) -> dict[str, Any]:
    return {
        ...
        "krikri_accuracy": nested_get(report, ("models", "krikri", "overall", "accuracy")),
        ...
        "krikri_vs_base_accuracy_delta": nested_get(
            report,
            ("comparison", "krikri_vs_base", "overall_accuracy_delta"),
        ),
        "trained_vs_krikri_accuracy_delta": nested_get(
            report,
            ("comparison", "trained_vs_krikri", "overall_accuracy_delta"),
        ),
...
def write_summary_files(output_dir: Path, records: Sequence[dict[str, Any]]) -> None:
    ...
    columns = [
        ...
        "krikri_vs_base_accuracy_delta",
        "trained_vs_krikri_accuracy_delta",
        ...
        lines.append(
            "\t".join(
                [
                    ...
                    str(record["step"]),
                    format_float(record["base_accuracy"]),
                    format_float(record["krikri_accuracy"]),
                    format_float(record["overall_accuracy"]),
                    format_float(record["overall_accuracy_delta"]),
                    format_float(record["krikri_vs_base_accuracy_delta"]),
                    format_float(record["trained_vs_krikri_accuracy_delta"]),
