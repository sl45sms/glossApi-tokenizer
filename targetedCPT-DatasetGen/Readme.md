...
# Targeted CPT Dataset Generation
this script generates a targeted CPT dataset for the Greek language, which is used to CPT test the model's ability to learn new tokens and their context. The dataset is generated by filtering the FineWeb2-HQ Greek corpus for documents that contain specific target words,previously selected for tokenizer insertion.


* Train with the full ~80GB `ell_Grek` corpus from `epfml/FineWeb2-HQ` gives the best result, but it is slow for quick iteration.Look the following table for a rough estimate of the time and quality trade-offs between the full CPT dataset and smaller targeted datasets.
...
After the creation of jsonl file, you can use the `prepare_cpt_dataset.py` script to prepare the dataset for training. This script will mix the filtered Greek documents with some English documents from the FineWeb-HQ dataset, group them into sequences, and write them in parquet format for efficient loading during training.
