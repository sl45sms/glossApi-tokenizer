...
def build_command(samples: List[str], base_tokenizer: str, reference_tokenizer: str, report_path: Path) -> List[str]:
    command = [
        "python",
        str(COMPARE_SCRIPT),
        "--trust-remote-code",
...
def compare_text(text: str, base_tokenizer: str, reference_tokenizer: str):
    ...
    command = build_command(samples, base_tokenizer, reference_tokenizer, report_path)
...
def create_app() -> gr.Blocks:
    with gr.Blocks(title="Tokenizer Visualizer") as app:
        gr.Markdown(
            ...
            "Compare Greek tokenization between Apertus and Krikri by sending one sentence per line."
        )
        ...
        compare_button.click(
            fn=compare_text,
            inputs=[text_input, base_tokenizer, reference_tokenizer],
