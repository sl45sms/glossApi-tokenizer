...
#SBATCH --output=/iopsstor/scratch/cscs/%u/logs/retok_cpt_%j.out
#SBATCH --error=/iopsstor/scratch/cscs/%u/logs/retok_cpt_%j.err
#SBATCH --mail-type=FAIL,END
#SBATCH --mail-user=panagiotis@skarvelis.gr
#
# ── Stage 2b: Full CPT Run ─────────────────────────────────────────────
#
# Continues pretraining on the retok-initialized checkpoint.
# 90% Greek (FineWeb2-HQ ell_Grek) + 10% English (FineWeb-HQ).
#
# Prerequisites:
...
OUTPUT_DIR="${CAPSTOR_SCRATCH_ROOT}/apertus-greek-cpt-retok"
RUN_NAME="apertus-greek-cpt-retok"
...
# ── CPT settings ───────────────────────────────────────────────────────
# Adjust these to control training length and batch size.
# Default: 50000 steps, global batch 256, seq len 2048.
...
# gradient_accumulation = target_global_batch / (per_device_batch * n_gpus)
GRADIENT_ACCUMULATION_STEPS=$(( TARGET_GLOBAL_BATCH_SIZE / (PER_DEVICE_TRAIN_BATCH_SIZE * NPROC_PER_NODE) ))
...
# Validate prepared dataset (optional but recommended for speed)
...
echo "========================================="
echo " Retok CPT FULL — $(date)"
echo " Model:     ${MODEL_PATH}"
echo " Output:    ${OUTPUT_DIR}"
echo " Steps:     ${WARMUP_MAX_STEPS} warmup + ${FULL_MAX_STEPS} full"
echo " Seq len:   ${MAX_SEQ_LENGTH}"
echo " Batch:     ${PER_DEVICE_TRAIN_BATCH_SIZE}x${NPROC_PER_NODE} GPUs x ${GRADIENT_ACCUMULATION_STEPS} accum = ${TARGET_GLOBAL_BATCH_SIZE} global"
...
if [[ -n "${PREPARED_TRAIN_DATASET_DIR}" ]]; then
	...
	echo " Dataset:   streaming (epfml/FineWeb2-HQ ell_Grek + FineWeb-HQ)"
fi
echo "========================================="
...
SRUN_EXPORT+=",OVERWRITE_OUTPUT_DIR=0"
...
echo "CPT training starting..." >&2
echo "Model path: ${MODEL_PATH}" >&2
echo "Output dir: ${OUTPUT_DIR}" >&2
echo "Global batch: ${TARGET_GLOBAL_BATCH_SIZE}" >&2
