...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--distill-reg-weight", type=float, default=0.1,
                        help="Embedding regularization weight: keeps new embeddings close to retok init (default: 0.1).")
    parser.add_argument("--distill-stream-timeout", type=int, default=600,
                        help="Max seconds for context collection before proceeding with what we have (default: 600).")
...
def _distill_core(
    *,
    ...
    w_in_init: Optional[torch.Tensor] = None,
    reg_weight: float = 0.0,
) -> int:
    """Shared distillation training loop with optional embedding regularization."""
    ...
    for step in range(start_step, distill_steps):
        ...
        if reg_weight > 0 and w_in_init is not None:
            # Penalize new embeddings drifting away from their retok initialization
            new_ids_list = sorted(new_ids_set)
            reg_loss = F.mse_loss(w_in[new_ids_list], w_in_init[new_ids_list].to(device))
...
def _collect_contexts_ahocorasick(
    ...
    # Ensure datasets library uses our cache directory
    _os.environ["HF_DATASETS_CACHE"] = fineweb2_cache
    ...
    found_total = 0
    save_interval = 500
    stream_timeout = args.distill_stream_timeout
    docs_scanned = 0

    print(f"Streaming FineWeb2-HQ (ell_Grek) — searching for {len(contexts_needed)} tokens "
          f"({CONTEXTS_PER_TOKEN} contexts each, timeout={stream_timeout}s)...")
    ...
    for example in ds:
        ...
        # Timeout: don't spend forever on rare tokens
        elapsed = _time.time() - t0
        if elapsed > stream_timeout:
            remaining = len(contexts_needed)
            print(f"  Stream timeout ({stream_timeout}s) reached. Proceeding with {found_total} contexts, "
                  f"{remaining} tokens without full contexts (will keep retok init).")
            for clean in list(contexts_needed.keys()):
                del contexts_needed[clean]
            break
...
def _distill_single(args, model, tokenizer, all_tokens_expanded, cache_dir):
    ...
    # ── Save retok initialization for regularization ──────────────────
    w_in_init = w_in.detach().clone().cpu() if args.distill_reg_weight > 0 else None
    ...
    _distill_core(
        ...
        w_in_init=w_in_init, reg_weight=args.distill_reg_weight,
    )
...
def _distill_worker(cfg_file: str):
    ...
    # ── Save retok initialization for regularization ──────────────────
    reg_weight = cfg.get("distill_reg_weight", 0.0)
    w_in_init = input_emb.detach().clone().cpu() if reg_weight > 0 else None
    ...
    _distill_core(
        ...
        grad_mask=grad_mask, new_ids_tensor=new_ids_tensor,
        w_in_init=w_in_init, reg_weight=reg_weight,
    )
...
def run_token_distillation(
    ...
    for i in range(num_gpus):
        ...
        worker_cfg = {
            ...
            "distill_reg_weight": args.distill_reg_weight,
        }
